...
知识库采用持续迭代开发模式，版本号（v1, v2, v3...）记录功能演进过程。所有版本修改最终合并到 `main` 分支，网站始终展示最新版本。
...
| 方面 | 说明 |
|------|------|
| 部署流程 | 保持不变，始终从 `main` 分支自动部署 |
| 配置文件 | `.github/workflows/deploy.yml` 无需修改 |
| 网站内容 | 始终展示最新版本的功能和内容 |
| 历史版本 | 通过 Git 历史记录查看，网站不保留旧版本 |

### 8.3 如需文档版本管理（可选）

MkDocs Material 支持文档版本管理（mike 插件），如需实现：
